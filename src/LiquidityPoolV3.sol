...
import "./interfaces/IUGIVaultExposure.sol";


contract LiquidityPoolV3 is AccessControl, ReentrancyGuard, Pausable {
    ...
    //IUGIVaultExposure public vault;
    IUGIVaultExposure public vault;
    ...
    function setVault(address newVault) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        vault = IUGIVaultExposure(newVault);
        //vault = IVaultExposure(newVault);
        ...
        returns (uint256 mintedShares)
    {
        ...
        if (totalShares == 0) {
            ...
            mintedShares = (received * totalShares) / totalAssets;
        }
        ...
        returns (uint256 amount)
    {
        ...
        require(shares[msg.sender] >= shareAmount, "INSUFFICIENT_SHARES");

        amount = (shareAmount * totalAssets) / totalShares;
        require(amount <= withdrawableLiquidity(), "LOCKED_LIQ");
    ...
    {
        require(amount > 0, "ZERO");
        require(amount <= withdrawableLiquidity(), "INSUFFICIENT_LIQ");
        ...
        whenNotPaused
    {
        ...
        require(totalBorrowed >= principal, "OVERPAY");
    ...
    function reservedLiquidity() public view returns (uint256) {
