    ...
    Pausable
{
    ...
    IUGINFTCoreV6 public immutable nft;
    ...
    uint256 public maxApprovalWindow = 15 minutes;
        ...
        EIP712("UGIOracle", "4")
    {
        ...
        require(_nft != address(0), "ZERO_NFT");

        vault = _vault;

        nft = IUGINFTCoreV6(_nft);
        ...
        )
    {
        ...
        Payload memory p =
            abi.decode(req.payload, (Payload));
        ...
        require(
            block.timestamp <=
            p.issuedAt + maxApprovalWindow,
            "STALE_APPROVAL"
        );

        require(
            nft.ownerOf(p.nftId) == req.user,
            "NOT_NFT_OWNER"
        );
        ...
        ) = nft.getReputation(p.nftId);

        require(!broken, "BROKEN_CHAIN");
